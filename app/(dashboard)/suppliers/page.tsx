...
export default function SuppliersPage() {
  ...
  const [editPhones, setEditPhones] = useState<SupplierPhone[]>([]);
  ...
  useRealtime("data:suppliers", fetchSuppliers);

  const handleSearch = (val: string) => {
    setSearch(val);
    if (searchTimerRef.current) clearTimeout(searchTimerRef.current);
    searchTimerRef.current = setTimeout(() => fetchSuppliers(val), 400);
  };

  const openEdit = (s: Supplier) => {
    setEditSupplier(s);
    setEditName(s.name);
    setEditPhones([...s.phones]);
    setNewPhone("");
  };
  ...
  const saveSupplierName = async () => {
    ...
    try {
      ...
      setSuppliers((prev) =>
        prev.map((s) =>
          s.id === editSupplier.id ? { ...s, name: editName.trim() } : s,
        ),
      );
  ...
  const addPhone = async () => {
    if (!editSupplier || !newPhone.trim()) return;
    try {
      setSavingPhone(true);
      await api.post(`/suppliers/${editSupplier.id}/phones`, {
        phone: newPhone.trim(),
      });
      toast.success("تم إضافة الرقم");
      const { data } = await api.get(`/suppliers/${editSupplier.id}/phones`);
      setEditPhones(data);
      setNewPhone("");
      setSuppliers((prev) =>
        prev.map((s) =>
          s.id === editSupplier.id ? { ...s, phones: data } : s,
        ),
      );
    } catch {
  ...
  const confirmDeletePhone = (
    ...
    setDeletePhoneTarget({ supplierId, phoneId, phone });
  };
  ...
  const deletePhone = async () => {
    ...
    try {
      await api.delete(
        `/suppliers/${deletePhoneTarget.supplierId}/phones/${deletePhoneTarget.phoneId}`,
      );
      ...
      setSuppliers((prev) =>
        prev.map((s) =>
          s.id === deletePhoneTarget.supplierId ? { ...s, phones: updated } : s,
        ),
      );
    } catch {
    ...
    <PageContainer>
      <div className="max-w-3xl mx-auto" dir="rtl">
        <div className="flex items-center justify-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-bold text-center">إدارة الموردين</h1>
        </div>
        <p className="text-sm text-muted-foreground text-center mb-6">
          عرض وتعديل بيانات الموردين وأرقام هواتفهم
        </p>
        ...
        {/* Edit Dialog */}
        <Dialog open={!!editSupplier} onOpenChange={(o) => !o && closeEdit()}>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <Input
                    ...
                    className="flex-1"
                    onKeyDown={(e) => {
                      if (e.key === "Enter") saveSupplierName();
                    }}
                  />
                  <Button
                    size="sm"
                    disabled={
                      savingName ||
                      !editName.trim() ||
                      editName.trim() === editSupplier?.name
                    }
                  ...
                  {editPhones.map((p) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          confirmDeletePhone(editSupplier!.id, p.id, p.phone)
                        }
                      >
